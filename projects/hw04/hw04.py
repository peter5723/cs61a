...
def balanced(m):
    ...
    if is_planet(m):
        return True
    if total_weight(end(left(m)))*length(left(m))==total_weight(end(right(m)))*length(right(m)):#read the question carefully please
...
def quadratic(x, a, b, c):
    ...
    def f(x):
        return a*x*x+b*x+c
    if a==0 or -b/(2*a)<lower_bound(x) or -b/(2*a)>upper_bound(x):
    ...
    if lower_bound(x) < -b/(2*a) <upper_bound(x):
        if a>0:
            return interval(f(-b/(2*a)),max(f(lower_bound(x)),upper_bound(x)))
        if a<0:
