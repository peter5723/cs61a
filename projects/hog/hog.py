...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while i <= num_rolls:
        res = dice()
        if res==1: index=False
        else: allsum += res
        i += 1
    if index:return allsum
    else:return 1
...
def more_boar(player_score, opponent_score):
    ...
    while opponent_score//100!=0:
        opponent_score//=10
    o2=opponent_score%10
    o1=opponent_score//10
    ...
    def say1(score0,score1):
        ...
        if(who==0):
            scorex=score0 
             ...
             scorex=score1
        if (scorex-ls>rh):
            print("Player",who,"has reached a new maximum point gain.",scorex-ls,"point(s)!")
            rh=scorex-ls
        ls=scorex
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    trial=[averaged_dice(i,dice) for i in range(1,11)]
    maxn=trial.index(max(trial))+1
    return maxn 
    #I do not know why it can pass but use while cannot
    # END PROBLEM 9
