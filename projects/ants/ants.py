...
class Ant(Insect):
    ...
    def buff(self):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self.have_buffed == False:
            self.damage = self.damage*2
            self.have_buffed=True
        if self.is_container():
            if self.contained_ant != None :
                if self.contained_ant.have_buffed == False:
                    self.contained_ant.damage = self.contained_ant.damage*2
                    self.contained_ant.have_buffed = True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        i=self.place
        now_range=0
        while now_range<self.min_range and not(i is beehive):
            now_range += 1
            i=i.entrance
        ...
        while not(i is beehive) and self.min_range<=now_range<=self.max_range:
            if i.bees!=[]:
                return bee_selector(i.bees)
            i=i.entrance
            now_range+=1
        return None
        #我不知道为什么条件一定要在父类这里写，不能在下面重载这个函数
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        list0 = self.place.bees.copy()
        for beee in list0:
            index0=self.place.bees.index(beee)
            Insect.reduce_health(self.place.bees[index0],amount)
            if self.health<=amount and beee in self.place.bees:
                Insect.reduce_health(self.place.bees[index0],self.damage)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chewing!=0:
            ...
            if self.place.bees!=[]:
                bee0 = bee_selector(self.place.bees)
                bee0.reduce_health(bee0.health)
...
class TankAnt(ContainerAnt):
    ...
    def action(self,gamestate):
        ...
        for beee in list0:
            index0 = self.place.bees.index(beee)
            self.place.bees[index0].reduce_health(self.damage)
...
class Bee(Insect):
    ...
    def slow(self, length):
        ...
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        self.apply_status(make_slow(self.action,self),self.action,length)
        # END Problem Optional 2
    ...
    def scare(self, length):
        ...
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        self.apply_status(make_scare(self.action,self),self.action,length)
        if self.apply_time == length:
            self.have_scared = True
        # END Problem Optional 2
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        i=self.place
        while i!=beehive:
            if (i.ant is not None and i.ant is not self):
                dic[i.ant]=dist
            if i.bees != []:
                for beee in i.bees:
                    dic[beee]=dist
